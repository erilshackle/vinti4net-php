...
<article class="vinti4-receipt">
    ...
    <dl class="vinti4-receipt__details">
        <div>
            <dt>Referência</dt>
            <dd><?= $escape($receipt['merchantReference']) ?></dd>
        </div>

        <?php if ($originalTransactionId !== null): ?>
            <div>
                <dt>Transação original</dt>
                <dd><?= $escape($originalTransactionId) ?></dd>
            </div>
        <?php endif; ?>

        <div>
            <dt>Transação de estorno</dt>
            <dd><?= $escape($receipt['transactionId']) ?></dd>
        </div>

        <div>
            <dt>Data/Hora</dt>
            <dd><?= $escape($receipt['timestamp']) ?></dd>
        </div>
