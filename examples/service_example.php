...
try {
    ...
    if (!preg_match('/^\d+$/', $entity) || !preg_match('/^\d{1,9}$/', $number)) {
        throw new Vinti4Exception('Entidade ou referência inválida.');
    }

    $vinti4 = new Vinti4Net($posId, $authCode, $endpoint);
    $vinti4->prepareServicePayment((string) ($_POST['amount'] ?? ''), (int) $entity, $number);
    ...
    error_log('Vinti4Net service example: ' . $exception->getMessage());
    echo '<h2>Não foi possível iniciar o pagamento.</h2><p><a href="/">Voltar</a></p>';
}
